ゲームは FPS (3D) で、プレイヤーは 遺跡（聖地）に入り、宝物 を探します。
宝物は 「見えないドア」 の中にあります。
ドアの部屋だけが見えなくて「透明」、他の場所は普通に見えます。

プレイヤーは 「ペイントボール」 を投げることができます。
ペイントが当たった場所は色がついて、形が見えるようになります。
だから、見えないドアにペイントを当てると、ドアの形だけうっすら見えるようになります。
このアイデアは 「The Unfinished Swan」ゲームに近い感じです。

でも、遺跡には 幽霊 がたくさんいて、幽霊もドアと同じで見えません（透明）。
ペイントが幽霊に当たると、その幽霊も形だけ見えるようになり、
その瞬間から幽霊はプレイヤーを追いかけて攻撃します。

（ペイントボールは何回でも投げられますが、幽霊はどこにでもいて見えないので、プレイヤーは「どこにペイントを投げるか」をよく考えないといけません。）

プレイヤーができることは2つです：

逃げる・隠れ
壁等の後ろに隠れて時間を稼ぐことができます。
でも、ゲームには 10分の時間制限 があり、
10分以内に宝物を見つけないとゲームオーバーです。
ずっと隠れることはできません。

攻撃して時間を止める
プレイヤーは 銀の弾（シルバーバレット） を撃つことができます。
また、斧 などの武器もあり、幽霊を「遅くする」「止める」ことができます。
ただし幽霊は死んでいるので、完全に倒すことはできません。

プレイヤーには 体力バー があり、幽霊の攻撃を受けすぎるとゲームオーバーです。

つまり、
ペイントでドアを探す、
幽霊を避ける／止める、
時間内に宝物を手に入れる
というゲームです。

プレイヤーが見えないドアを見つけて中に入ると、幽霊はもう追ってこなくなります。
でも、ドアから宝物までの間には、仕掛け を入れようと思っています。

*** add cutscenes ***
- starting cutscene, in which, that the player has come here in search of treasure and he has a map of this area and it something is written on it and that is "find the treasure before 8pm, because after that ghosts gonna immortal"
- before boss fight cutscene, just after teleporting inside the hidden room, boss ghost protecting the treasure, circling it and calling the player to come and fight
- after winning treasure cutscene, `
- death cutscene, 

*** add a mini map marking the playable area in a circle ***
*** give hints like sign boards or texts to find the invisible door ***
*** add tricks like, place arrow decals on some places just like for invisible door, but instead of invisible door, place there an invisible ghost ***

=> for ghost attack on player when ghost is near the player: 
    - stop player movement and ghost's animation, play the preanimated cutscene that a hand came out of the ghost's mouth and grabbed the camera and then lifting and throwing the player -> health damage/gmae over
    - ghost giving a Headbutt to the player and player took the damage and game over maybe

=> using behaviour trees for ghost AI